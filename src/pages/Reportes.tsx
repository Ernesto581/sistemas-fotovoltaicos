...
export default function Reportes() {
  ...
  const filas: Fila[] = socios.map((s) => {
    ...
    return {
      nombre: s.nombre,
      materialesMn: mats.reduce((a, m) => a + (m.cantidad || 0) * (m.precio_mn || 0), 0),
      materialesUsd: mats.reduce((a, m) => a + (m.cantidad || 0) * (m.precio_usd || 0), 0),
      gastosMn: gs.reduce((a, g) => a + (g.monto_mn || 0), 0),
      gastosUsd: gs.reduce((a, g) => a + (g.monto_usd || 0), 0),
    }
  })
      ...
      {todas.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-3 sm:hidden">
            {todas.map((f) => (
              <Card key={f.nombre} className="p-4">
                <div className="font-semibold text-slate-800">{f.nombre}</div>
                <div className="mt-2 space-y-1 text-sm">
                  <div className="flex justify-between">
                    <span className="text-slate-500">Materiales MN</span>
                    <span className="font-medium">{fmtMoney(f.materialesMn, 'MN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Materiales USD</span>
                    <span className="font-medium">{fmtMoney(f.materialesUsd, 'USD')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Gastos MN</span>
                    <span className="font-medium">{fmtMoney(f.gastosMn, 'MN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Gastos USD</span>
                    <span className="font-medium">{fmtMoney(f.gastosUsd, 'USD')}</span>
                  </div>
                </div>
              </Card>
            ))}
